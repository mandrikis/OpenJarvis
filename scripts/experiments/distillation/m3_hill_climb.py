...
Usage:
    python scripts/experiments/distillation/m3_hill_climb.py \\
...
def build_user_prompt(
    *,
    ...
    hist = (
        "\n".join(hist_lines)
        if hist_lines
        else "  (no edits yet — baseline is the starting point)"
    )
...
def apply_edit(cfg: Config, edit: dict, *, default_agent: str | None = None) -> Config:
    ...
    p = {
        **edit.get("params", {}),
        **{k: v for k, v in edit.items() if k not in ("op", "params", "rationale")},
    }
    new = Config(
...
def hill_climb(args) -> dict:
    ...
    if state_path.exists() and not args.fresh:
        ...
        if args.trust_baseline and args.baseline_score is not None:
            ...
            print(
                f"[m3] Trusting provided baseline score: {measured_baseline:.1f}% "
                f"(--trust-baseline set)"
            )
        else:
            print(
                f"[m3] Measuring today's baseline with k={args.k_final} "
            ...
            bl_toml = write_eval_toml(
                ...
                k_samples=args.k_final,
                output_dir=bl_dir,
            )
            bl_oj = write_openjarvis_config(
                bl_dir, student["port"], baseline_cfg_dict["max_turns"]
            )
            t0 = time.monotonic()
            measured_baseline, bl_scored, bl_total = run_eval(bl_toml, bl_oj)
            print(
                f"[m3] Measured baseline: {measured_baseline:.1f}% "
                f"({bl_scored}/{bl_total}) in {(time.monotonic() - t0) / 60:.1f} min"
            )
            if args.baseline_score is not None:
                print(
                    f"[m3] (reference: --baseline-score was {args.baseline_score:.1f}%, "
    ...
    for round_num in range(len(state["history"]) + 1, args.rounds + 1):
        ...
        try:
            edit = call_proposer(
                engine, PROPOSER_SYSTEM, user_prompt, model=args.proposer_model
            )
        except Exception as e:
        ...
        if edit.get("op") == "noop":
            print("[m3] Teacher proposed noop; stopping.")
            break
        ...
        eval_toml = write_eval_toml(
            ...
            student=student,
            cfg=candidate,
            k_samples=args.k_subsample,
            output_dir=round_dir,
        )
        oj_cfg = write_openjarvis_config(
            round_dir, student["port"], candidate.max_turns
        )
        ...
        print(
            f"[m3] Subsample score: {acc:.1f}% ({scored}/{total}) in {elapsed / 60:.1f} min"
        )
    ...
    print(
        f"[m3] final    = {final_acc:.1f}% ({final_scored}/{final_total}) in {elapsed / 60:.1f} min"
    )
